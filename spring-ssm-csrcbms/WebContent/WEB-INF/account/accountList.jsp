    ...
    <head>
        ...
        <script language="javascript" type="text/javascript">
            ...
            function start_account(id) {
            	...
            	if(r) {
	            	window.location.href = "startAccount?accountId="+id;
            	}
            }
            //暂停
            function pause_account(id) {
            	...
            	if(r) {
	            	window.location.href = "pauseAccount?accountId="+id;
            	}
            }
            //删除
    ...
    <body>
        ...
        <div id="header">
            <img src="../images/logo.jpg" alt="logo" class="left"/>
             <a href="<%=request.getContextPath() %>/login/toLogin">[退出]</a>             
        </div>
        <!--Logo区域结束-->
        <!--导航区域开始-->
        <div id="navi">   
        ...
        <div id="main">
            <form action="findAccount" method="post">
	                        ...
	                        <td class="td_modi">
	                        	<c:choose>
	                        		<c:when test="${a.status==0 }">
			                            ...
			                            <input type="button" value="修改" class="btn_modify" onclick="location.href='toUpdateAccount?accountId=${a.accountId }';" />
	                        		...
	                        		<c:when test="${a.status==1 }">
			                            ...
			                            <input type="button" value="修改" class="btn_modify" onclick="location.href='toUpdateAccount?account_id=${a.accountId }';" />
			                            <input type="button" value="删除" class="btn_delete" onclick="delete_account(${a.accountId });" />
	                        		</c:when>
	                        		<c:otherwise>
                ...
                <!--分页-->
                <div id="pages">
                    <a href="findAccount?currentPage=1">首页</a>
                	<c:choose>
                		<c:when test="${accountPage.currentPage==1 }">
                			<a href="#">上一页</a>
                		</c:when>
                		<c:otherwise>
                			<a href="findAccount?currentPage=${accountPage.currentPage-1 }">上一页</a>
                		</c:otherwise>
                	</c:choose>
        	        ...
        	        <c:forEach begin="1" end="${accountPage.totalPage }" var="p">
        	        	<c:choose>
        	        		<c:when test="${p==accountPage.currentPage }">
        	        			<a href="findAccount?currentPage=${p }" class="current_page">${p }</a>
        	        		</c:when>
        	        		<c:otherwise>
        	        			<a href="findAccount?currentPage=${p }">${p }</a>
        	        		</c:otherwise>
        	        	</c:choose>
                    </c:forEach>
                    ...
                    <c:choose>
                		...
                		<c:otherwise>
                			<a href="findAccount?currentPage=${accountPage.currentPage+1 }">下一页</a>
                		</c:otherwise>
                	</c:choose>
                    <a href="findAccount?currentPage=${accountPage.totalPage }">末页</a>
                </div>                    
            </form>
        </div>
        <!--主要区域结束-->
