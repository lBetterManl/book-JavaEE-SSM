    ...
    <body>
        ...
        <div id="main">
            <form action="findService" method="post">
	                        ...
	                        <td class="td_modi">
	                        	<c:choose>
	                        		<c:when test="${service.status==0 }">
			                            ...
			                            <input type="button" value="修改" class="btn_modify" onclick="location.href='toUpdateService?id=${service.serviceId}';" />
                ...
                <!--分页-->
                <div id="pages">
                    <a href="findService?currentPage=1">首页</a>
                	<c:choose>
                		<c:when test="${servicePage.currentPage==1 }">
                			<a href="#">上一页</a>
                		</c:when>
                		<c:otherwise>
                			<a href="findService?currentPage=${servicePage.currentPage-1 }">上一页</a>
                		</c:otherwise>
                	</c:choose>
        	        ...
        	        <c:forEach begin="1" end="${servicePage.totalPage }" var="p">
        	        	<c:choose>
        	        		<c:when test="${p==servicePage.currentPage }">
        	        			<a href="findService?currentPage=${p }" class="current_page">${p }</a>
        	        		</c:when>
        	        		<c:otherwise>
        	        			<a href="findService?currentPage=${p }">${p }</a>
        	        		</c:otherwise>
        	        	</c:choose>
                    </c:forEach>
                    ...
                    <c:choose>
                		...
                		<c:otherwise>
                			<a href="findService?currentPage=${servicePage.currentPage+1 }">下一页</a>
                		</c:otherwise>
                	</c:choose>
                    <a href="findService?currentPage=${servicePage.totalPage }">末页</a>
                </div>                    
            </form>
        </div>
        <!--主要区域结束-->
